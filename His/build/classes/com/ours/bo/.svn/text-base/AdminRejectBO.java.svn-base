...
public class AdminRejectBO implements QueryInterface{
	static final Logger LOGGER = Logger.getLogger(Login.class);
public Map<Integer, PolicyBean> rejectPolicy(){
	
		int count=1;
		Map<Integer, PolicyBean> mymap = new HashMap<Integer,PolicyBean>();
		Connection con = DAO.getConnection();
		try {
			PreparedStatement ps = con.prepareStatement(rejectedPolicy);
			...
			LOGGER.error("SQLException in AdminRejectBo.rejectPolicy : "+e);
		}
		return mymap;
